...
pub async fn run(args: &IndexArgs) -> Result<()> {
    ...
    let embedder = embed::EmbedClient::new(Some(&root_path), embedding_config.clone())?;
    ...
    db::upsert_project_config(&pool, &project, &embedding_config).await?;
...
async fn resolve_index_embedding_config(
    pool: &sqlx::PgPool,
    project: &str,
    args: &IndexArgs,
) -> Result<embed::EmbeddingConfig> {
    let default = embed::EmbeddingConfig::default_openai();

    if let Some(spec) = args.embedding.as_deref() {
        let requested = embed::EmbeddingConfig::parse(spec)?;
        if db::get_project_config(pool, project).await?.is_some()
            || db::project_has_indexed_data(pool, project).await?
        {
            return db::ensure_project_config(pool, project, &requested).await;
        }
        return Ok(requested);
    }

    if let Some(stored) = db::get_project_config(pool, project).await? {
        return Ok(stored);
    }

    if db::project_has_indexed_data(pool, project).await? {
        return db::resolve_project_config(pool, project, &default).await;
    }

    let env_config = std::env::var("SCANR_EMBEDDING")
        .ok()
        .filter(|v| !v.trim().is_empty())
        .map(|spec| embed::EmbeddingConfig::parse(&spec))
        .transpose()?;

    Ok(env_config.unwrap_or(default))
}
