...
/// Safety limit for chunk size before embedding. `OpenAI` embedding models
...
impl EmbeddingConfig {
    ...
    pub fn parse(spec: &str) -> Result<Self> {
        ...
        let Some((provider, model)) = spec.split_once(':') else {
            anyhow::bail!(
                "Invalid embedding `{spec}`. Expected provider:model, for example `openai:text-embedding-3-large`."
            );
        };
        ...
        match (provider, model) {
            ("openai", "text-embedding-3-large") => Ok(Self::openai_large()),
            ("openai", "text-embedding-3-small") => Ok(Self::openai_small()),
            ("openai", _) => anyhow::bail!(
                "Unsupported OpenAI embedding model `{model}`. Supported values: `openai:text-embedding-3-large`, `openai:text-embedding-3-small`."
            ),
            _ => anyhow::bail!(
                "Unsupported embedding provider `{provider}`. Supported values: `openai:text-embedding-3-large`, `openai:text-embedding-3-small`."
            ),
        }
    }
}
...
fn validate_supported_config(config: &EmbeddingConfig) -> Result<()> {
    if config.provider != "openai" {
        anyhow::bail!(
            "Unsupported embedding provider `{}`. This build currently supports only OpenAI.",
            config.provider
        );
    }

    let expected = match config.model.as_str() {
        "text-embedding-3-large" => 2000,
        "text-embedding-3-small" => 1536,
        _ => {
            anyhow::bail!(
                "Unsupported embedding config `{}`. This build currently supports only OpenAI `text-embedding-3-large` and `text-embedding-3-small`.",
                config.model
            );
        }
    };

    if config.dimensions != expected {
        anyhow::bail!(
            "Unsupported embedding config `{}` ({}d). Expected {} dimensions for this model.",
            config.model,
            config.dimensions,
            expected
        );
...
pub fn pad_embedding_for_storage(embedding: &[f32]) -> Result<Vec<f32>> {
    if embedding.len() > STORAGE_DIMENSIONS as usize {
        anyhow::bail!(
            "Embedding has {} dimensions, which exceeds the storage limit of {}.",
            embedding.len(),
            STORAGE_DIMENSIONS
        );
    }
    ...
    padded.resize(STORAGE_DIMENSIONS as usize, 0.0);
    Ok(padded)
}
...
mod tests {
    ...
    #[test]
    fn parses_openai_large_embedding_spec() {
        let config = EmbeddingConfig::parse("openai:text-embedding-3-large").unwrap();
        assert_eq!(config, EmbeddingConfig::openai_large());
    }

    #[test]
    fn parses_openai_small_embedding_spec() {
        let config = EmbeddingConfig::parse("openai:text-embedding-3-small").unwrap();
        assert_eq!(config, EmbeddingConfig::openai_small());
    }

    #[test]
    fn rejects_invalid_embedding_spec_format() {
        let err = EmbeddingConfig::parse("text-embedding-3-large").unwrap_err().to_string();
        assert!(err.contains("Expected provider:model"));
    }

    #[test]
    fn rejects_unsupported_embedding_provider() {
        let err = EmbeddingConfig::parse("ollama:nomic-embed-text").unwrap_err().to_string();
        assert!(err.contains("Unsupported embedding provider"));
    }

    #[test]
    fn pad_embedding_preserves_prefix_and_zero_fills() {
        let padded = pad_embedding_for_storage(&[1.0, 2.0, 3.0]).unwrap();
        assert_eq!(padded.len(), STORAGE_DIMENSIONS as usize);
        assert_eq!(&padded[..3], &[1.0, 2.0, 3.0]);
        assert!(padded[3..].iter().all(|v| *v == 0.0));
    }

    #[test]
    fn pad_embedding_rejects_oversized_vectors() {
        let oversized = vec![0.0; STORAGE_DIMENSIONS as usize + 1];
        let err = pad_embedding_for_storage(&oversized).unwrap_err().to_string();
        assert!(err.contains("exceeds the storage limit"));
    }
